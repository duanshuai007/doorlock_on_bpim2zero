	...
	def test_publish(self):
		...
		while True:
			...
			if self.publish_queue is not None:
				...
				ms.OPENSSH_INFO["enable"] = int(self.enable_ssh)
				ms.OPENSSH_INFO["opentime"] = 300
				sendmsg = json.dumps(ms.OPENSSH_INFO)
				self.publish_queue.put({"topic":ms.OPENSSH_TOPIC, "payload":sendmsg, 'qos':2, 'retain':False})
				time.sleep(10)
				break
...
if __name__ == "__main__":
	...
	mc = mqtt_client(	client_id = "id_shenyang_test_duanshuai_9789797", 
						...
						transport = 'tcp')
	mc.setsubscribe(topic=ms.OPENSSH_RESP_TOPIC, qos=2)
