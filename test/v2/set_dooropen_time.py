...
class mqtt_client(mqtt.Client):
	...
	def on_connect(self, mqttc, obj, flags, rc):
		...
		try:
			qr_generate2vcode = {
				...
				"time" : int(time.time()),
				"message" : {
					"config" : self.dooropen_time,
				},
			}
			sendmsg = json.dumps(qr_generate2vcode)
			topic = "{}{}".format("/ctrl/", self.device_sn)
			self.publish_queue.put({"topic":topic, "payload":sendmsg, 'qos':0, 'retain':False})
	...
	def set_deviceid_and_dooropen_time(self, devid, dotime):
		self.device_sn = devid
		self.dooropen_time = dotime
...
-----END CERTIFICATE-----
	...
	mc.set_deviceid_and_dooropen_time(devid, opentime)
