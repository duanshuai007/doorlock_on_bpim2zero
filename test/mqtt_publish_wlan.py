...
class mqtt_client(mqtt.Client):
	...
	def on_connect(self, mqttc, obj, flags, rc):
		...
		if self.publish_queue is not None:
			sendmsg = json.dumps(wlaninfo)
			self.publish_queue.put({"topic":"/wlan_config", "payload":sendmsg, 'qos':0, 'retain':False})
	...
	def set_devicesn_and_wlan(self, device_sn, ssid, psk):
...
if __name__ == "__main__":
	...
	if len(sys.argv) < 4:
		print("like this:")
		print("python3 xxx.py 0242fe007c52 [ssid] [psk]")
		exit(1)
	...
	mc.set_devicesn_and_wlan(device_id, ssid, psk)
